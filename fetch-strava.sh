...
# Filter for Run activities, sum distance (meters → miles) for week and month
WEEK_DISTANCE_MI=$(echo "$ACTIVITIES" | jq --argjson after "$WEEK_START_UNIX" \
  '[.[] | select(.type == "Run") | select((.start_date | fromdateiso8601) >= $after) | .distance] | add // 0 | . / 1609.344 | . * 10 | round / 10')
